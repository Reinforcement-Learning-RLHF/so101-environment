...
class RandomizedIKPolicy:
    def __init__(self, env):
        ...
        self.offsets = [
            xyz - (self.SOURCE_CUP_HOME if i <= 2 else self.TARGET_CUP_HOME)
            for i, xyz in enumerate(self.calibrated_xyz)
        ]

        self.keyframe_steps = [30, 30, 20, 30, 30]
        self.reset()
    ...
    def get_action(self, obs):
        if self.current_frame >= len(self.joint_targets_rad):
            return self.joint_targets_rad[-1]
        ...
        if self.step_in_frame == 0:
            self.start_qpos_rad = obs["agent_pos"][:6].copy()
